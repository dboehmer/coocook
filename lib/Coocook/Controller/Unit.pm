...
sub create : Local : POST {
	...
	my $input_okay = $self->check_input($c, {short_name => $short_name, long_name => $long_name, to_quantity_default => $to_quantity_default, current_page => '/units'});
	if($input_okay){
...
sub update : Local : Args(1) : POST {
	...
	my $long_name = scalar $c->req->param('long_name');
	my $to_quantity_default = scalar $c->req->param('to_quantity_default');
    my $input_okay = $self->check_input($c, {short_name => $short_name, long_name => $long_name, to_quantity_default => $to_quantity_default, current_page => "/unit/$id"});
    if($input_okay){
        $unit->update(
        {
            short_name => $short_name,
...
sub check_input : Private {
    ...
    if (length($shortname) <= 0){
		...
		$c->response->redirect(
               $c->uri_for( $current_page, { error => "Please supply a valid number in 'Factor to quantity's default unit'.\n E.g.: 1.33" } ) );
	} else {
        $result = 1;
    }
    return $result;
