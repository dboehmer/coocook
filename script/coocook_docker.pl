...
if ( defined $ARGV[0]) {
    ...
    say warning("You provided no command to run. You probably meant to run " . colored( "'$0 serve --debug --reload'", 'blue') . '?');
    say help();
    exit(1);
...
sub usage {
	...
	--install    create new database
	--upgrade    upgrade existing database to fit new schema
	},
